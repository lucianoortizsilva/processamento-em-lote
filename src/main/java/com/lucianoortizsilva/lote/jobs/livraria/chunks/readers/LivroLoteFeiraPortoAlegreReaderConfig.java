...
@Configuration
public class LivroLoteFeiraPortoAlegreReaderConfig {

	@Bean
	FlatFileItemReader<LivroDTO> livroLoteFeiraPortoAlegreReader() {
		return new FlatFileItemReaderBuilder<LivroDTO>()//
				.name("livroLoteFeiraPortoAlegreReader")//
				.resource(new FileSystemResource("arquivos/livros_lote_feira_porto_alegre.csv"))//
				.delimited().delimiter(",")//
				.names(getNames())//
